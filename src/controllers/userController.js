...
module.exports = {
    ...
    deleteUser : async (req, res) => {
        try {
            let {userId} = req.params
            let deleteData = await userModel.findOneAndUpdate({_id: userId, isDeleted: false},{isDeleted: true})
            if (!deleteData) {
                return res.status(404).send({ status: false, msg: "User not found" })
            }
    ...
    removeUser : async (req, res) => {
        try {
            let {userId, bookId, removeUserId} = req.params
            let removedData = await bookModel.findOneAndUpdate({_id: bookId, userId: userId, isDeleted: false}, {$push: {removedUser: removeUserId}})
            if (!removedData) {
                return res.status(404).send({ status: false, msg: "User not found" })
            }
            return res.status(200).send({ status: true, msg: 'User removed from system successfully'})
        } catch (error) {
            return res.status(500).send({ status: false, message: error.message })
        }
    },
